...
class VideoClassificationLightningModule(pytorch_lightning.LightningModule):
    def __init__(self, args):
        ...
        if self.args.arch == "rusnet":
            self.model = rusnet.SlowFast_rus(num_class=model_num_class)
        ...
        if self.args.arch == 'vivit':
            self.model = vivit.ViViT((224, 224), patch_size=16, num_frames=32, num_ego_class=0, num_actor_class=model_num_class)
            self.batch_key = "video"
            self.model.to(device="cuda")

        if self.args.arch == 'mvit':
            self.model = mvit.MViT(num_ego_class=0, num_actor_class=model_num_class, scale=-1.0)
        ...
        if self.args.arch == 'videoMAE':
            ...
            self.model = mvit.VideoMAE_pretrained(video_mae=self.model, num_ego_class=0, num_actor_class=model_num_class)
        ...
        if self.args.arch == 'ARG' or self.args.arch == 'ORN':
            if self.args.arch == 'ARG':
                self.model = ARG.ARG(args, max_N=80, num_ego_class=0, num_actor_class=model_num_class)
            else:
                self.model = ORN.ORN(args, max_N=80, num_ego_class=0, num_actor_class=model_num_class)
            for t in self.model.parameters():
                t.requires_grad=True
    ...
    def training_step(self, batch, batch_idx): #not bottle neck
        original_input = batch[self.batch_key]
        original_label = torch.stack(batch["label"]).t()
        ...
        ### Loss Calculation ###
        pos_lambda = torch.tensor([1],device=y_hat.device) #effect is 1 => +100%
        loss = 0
        #loss calc
        ...
        if self.args.arch == "rusnet":
            ...
            if self.current_epoch <=10:
                loss = 0.1*weighted_main_loss.mean() + 0.1*sub_loss.mean() + tri_loss.mean()
            elif self.current_epoch <=30:
                loss = 0.1*weighted_main_loss.mean() + sub_loss.mean()# + 0.1*tri_loss.mean()
            else:
    ...
    def validation_step(self, batch, batch_idx): #not neck
        ...
        y_pred_binary = (y_hat > 0.5).float().cpu()
...
def main():
    ...
    parser.add_argument(
        "--arch",
        default="slowfast",
        choices=["slowfast", "csn", "x3d","i3d","ARG","ORN","vivit","mvit",'videoMAE','rusnet'], #ARG #ORN cannot be used
        type=str,
    )
    parser.add_argument('--backbone', default="x3d-2", type=str)

    # Data parameters.tnf4_vf2
    parser.add_argument("--data_path", default="/home/magecliff/Traffic_Recognition/newframe4/labels_tnf4_vnf4", type=str) #Label
    parser.add_argument("--video_path_prefix", default="/home/magecliff/Traffic_Recognition/newframe4/videos", type=str) #video 
