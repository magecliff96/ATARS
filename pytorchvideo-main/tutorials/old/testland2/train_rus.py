...
class VideoClassificationLightningModule(pytorch_lightning.LightningModule):
    ...
    def test_step(self, batch, batch_idx): #not neck
        ...
        # Compute mean average precision #precision micro and mAP here differs by rounding
        mAP = np.mean(ap_scores) if ap_scores else 0
        if mAP > getattr(self, 'best_batch_test_mAP', 0):  
            self.best_batch_test_mAP = mAP
        self.log("best_batch_test_mAP", self.best_batch_test_mAP, on_step=False, on_epoch=True)
        self.log('test_mAP', mAP, prog_bar=True)

        y_pred_binary = (y_hat > 0.5).float().cpu().numpy()
        precision_micro = precision_score(batch_labels_array, y_pred_binary, average='micro', zero_division=0)
        acc = self.test_accuracy(y_hat, batch_labels_tensor.to(torch.int64))
        iteration = self.trainer.current_epoch * len(self.trainer.datamodule.test_dataloader()) + batch_idx
        test_writer.add_scalar("test/precision_micro", float(precision_micro), iteration)
        test_writer.add_scalar("test/loss", float(loss), iteration)
        test_writer.add_scalar("test/acc", float(acc), iteration)
        test_writer.add_scalar("test/mAP", float(mAP), iteration)
...
class BestMAPCallback(pytorch_lightning.Callback):
    ...
    def on_validation_end(self, trainer, pl_module):
        ...
        if self.monitor_val in trainer.callback_metrics:
            ...
            if current_mAP > self.val_best_mAP:
...
def main():
    ...
    parser.add_argument(
        "--arch",
        default="slowfast",
        choices=["slowfast", "csn", "x3d","i3d","ARG","ORN","vivit","mvit",'videoMAE','action_slot',"rusnet"], #ARG #ORN cannot be used
        type=str,
    )
