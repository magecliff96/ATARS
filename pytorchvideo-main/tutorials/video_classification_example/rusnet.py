...
class RusNet(nn.Module):
    def __init__(self, num_actor_class):
        ...
        # self.vote = nn.Parameter(torch.rand(num_actor_class))
    ...
    def forward(self, x, optic):
        ...
        if isinstance(optic, list):
            ...
            # l, b, c, h, w
            optic = optic.permute(1,2,0,3,4) #[b, v, 2048, h, w]
            slow_optic = slow_optic.permute(1,2,0,3,4)
        num_block = len(self.model.blocks)

        sub = [slow_x, x]#originally x
        main = [slow_optic, optic]#originally optic

        for i in range(num_block-1):
            sub = self.optic_model.blocks[i](sub)
            main = self.model.blocks[i](main)

            if i!=5:
                b0, c0, t0, h0, w0 = main[0].size()#slow
                b1, c1, t1, h1, w1 = main[1].size()#fast

                optic_auxW_slow= self.CA_optic_slow[i](sub[0]).view(b0, c0, 1, 1, 1)
                optic_auxW_fast= self.CA_optic_fast[i](sub[1]).view(b1, c1, 1, 1, 1)

                sub[0] = self.layernorms_slow_optic[i]((sub[0] + sub[0] * optic_auxW_slow))
                sub[1] = self.layernorms_fast_optic[i]((sub[1] + sub[1] * optic_auxW_fast))

                x_auxW_slow = self.ICA_slow[i](main[0]).view(b0, c0, 1, 1, 1) + optic_auxW_slow
                x_auxW_fast = self.ICA_fast[i](main[1]).view(b1, c1, 1, 1, 1) + optic_auxW_fast

                main[0] = self.layernorms_slow[i]((main[0] + main[0] * x_auxW_slow))
                main[1] = self.layernorms_fast[i]((main[1] + main[1] * x_auxW_fast))

        main = self.pool(main)
        main = torch.reshape(main, (batch_size, -1))
        main = self.model.blocks[-1](main) #[8,2304]

        sub = self.pool(sub)
        sub = torch.reshape(sub, (batch_size, -1))
        sub = self.optic_model.blocks[-1](sub) #problem [8,2304]
        ## work left to do => incorperate pathway exchange
        # vote_norm = torch.sigmoid(self.vote)
        # output = vote_norm * main + (1 - vote_norm) * sub
        return main, sub #swapped
