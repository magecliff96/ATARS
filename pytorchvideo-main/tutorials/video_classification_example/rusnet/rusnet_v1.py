    ...
    def forward(self, x, optic):
        ...
        for i in range(num_block-1):
            ...
            if i!=5:
                ...
                # slow_a = torch.sigmoid(self.alpha_slow[i])
                # fast_a = torch.sigmoid(self.alpha_fast[i])
                # optic[0] = self.layernorms_slow_optic[i](optic[0] + slow_a * optic[0] * optic_auxW_slow)
                # optic[1] = self.layernorms_fast_optic[i](optic[1] + fast_a * optic[1] * optic_auxW_fast)
                ...
                # slow_b = torch.sigmoid(self.beta_slow[i])
                # fast_b = torch.sigmoid(self.beta_fast[i])
                # x[0] = self.layernorms_slow[i](x[0] + slow_b * x[0] * x_auxW_slow)
                # x[1] = self.layernorms_fast[i](x[1] + fast_b * x[1] * x_auxW_fast)
