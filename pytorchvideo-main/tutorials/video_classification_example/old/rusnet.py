...
#         for i in range(num_block-1):
#             x = self.model.blocks[i](x)
#             if i == (num_block-2): #added
#                 x_base = self.pool(x)
#                 x_base = torch.reshape(x_base, (batch_size, -1))
#         x = self.pool(x)
#         x = torch.reshape(x, (batch_size, -1))

#         x, x_sub = self.model.blocks[-1](x)
#         x_tri = self.fc_tri(x_base)
#         #sub
#         sub_weight = torch.sigmoid(self.fc_tri_to_sub(x_tri))
#         x_sub = x_sub * sub_weight
#         #pri
#         pri_weight = torch.sigmoid(self.fc_sub_to_pri(x_sub))
#         x = x * pri_weight
#         return x, x_sub, x_tri
