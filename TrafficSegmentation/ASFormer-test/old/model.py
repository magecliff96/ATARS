...
class MyTransformer(nn.Module):
    ...
    def forward(self, x):
        #added
        _,t_initial,_,_,_ = x.shape
        x = x.permute((0,2,1,3,4)) #[b, v, 2048, h, w]

        num_block = len(self.pretrain.blocks)
        for i in range(num_block-1):
            x = self.pretrain.blocks[i](x)
        x=self.global_avg_pool(x)
        b, c, t, h, w = x.shape

        x = x.view(b, c, t * h * w)
        x = F.interpolate(x, size=t_initial, mode='nearest')

        b,c,k = x.shape
        mask = torch.zeros(len(x), self.num_classes, k , dtype=torch.float)
        x_new = torch.zeros(len(x), np.shape(x[0])[0], t_initial, dtype=torch.float)  # bs, C_in, L_in
        for i in range(len(x)):
            x_new[i, :, :x[i].shape[1]] = x[i]
            mask[i, :, :t_initial] = torch.ones(self.num_classes, t_initial)
        mask = mask.to(device)
        x = x_new.to(device)
        #added
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, batch_gen_tst=None):
        ...
        for epoch in range(num_epochs):
            ...
            while batch_gen.has_next():
                batch_input, batch_target, vids = batch_gen.next_batch(batch_size, False)

                batch_input, batch_target = batch_input.to(device), batch_target.to(device)
                optimizer.zero_grad()
                ps, mask = self.model(batch_input)
                mask = mask.to(device)
                ...
                prediction = torch.sigmoid(p.transpose(2, 1).contiguous().view(-1, self.num_classes))
                prediction = prediction.round()
                batch_gt = batch_target.view(-1, self.num_classes); mask = mask.view(-1, self.num_classes)
                # print("Shape of prediction before reshape:", prediction.shape)
                # print("Shape of batch_target before reshape:", batch_gt.shape)
                correct += ((prediction == batch_gt).float() * mask).sum().item()
    ...
    def test(self, batch_gen_tst, epoch):
        ...
        with torch.no_grad():
            while batch_gen_tst.has_next():
                batch_input, batch_target, vids = batch_gen_tst.next_batch(1, if_warp)
