...
from UVAST_parts.bmvc_advanced import Encoder_advanced
from UVAST_parts.bmvc_enc_dec import Decoder_org_enc_dec, Encoder_org_enc_dec
from UVAST_parts.bmvc_org import Encoder_org
from UVAST_parts.PE import SinusoidalPositionalEmbedding
from UVAST_parts.segment_embedding import SegmentEmbedding
from UVAST_parts.utils import exponential_descrease, remove_duplicates_from_transcript 
from UVAST_parts.uvast_dec import TransformerDecoder_UVAST, TransformerDecoderLayer_UVAST
...
class encoder_asformer_org_enc(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.enc = Encoder_org(num_layers=10, r1=2, r2=2, num_f_maps=64, input_dim=2048, num_classes=40, channel_masking_rate=0.3, att_type='sliding_att', alpha=1, device=args.device)
...
class encoder_asformer_advanced_enc(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.enc = Encoder_advanced(num_layers=10, r1=2, r2=2, num_f_maps=64, input_dim=2048, num_classes=40, channel_masking_rate=0.3, att_type='sliding_att', args=args)
...
class encoder_asformer_org_enc_dec(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.enc = Encoder_org_enc_dec(num_layers=40, r1=2, r2=2, num_f_maps=64, input_dim=2048, num_classes=40, channel_masking_rate=0.3, att_type='sliding_att', alpha=1, device=args.device)
        self.dec = nn.ModuleList([copy.deepcopy(Decoder_org_enc_dec(num_layers=10, r1=2, r2=2, num_f_maps=64, input_dim=40, num_classes=40, att_type='sliding_att', alpha=exponential_descrease(s), device=args.device)) for s in range(args.num_layers_asformer_dec_repeat)])
...
class decoder_duration(nn.Module):
    def __init__(self, args):
        ...
        if self.args.alignment_decoder_model == 'uvast_decoder':
            self.decoder_duration = TransformerDecoder_UVAST(TransformerDecoderLayer_UVAST(d_model=64, nhead=1, activation='gelu', 
                                                                                           dropout=0.1, dim_feedforward=1024, args=args), 
                                                                                           repeat_mod=1, num_layers=1, args=args)
        elif self.args.alignment_decoder_model == 'pytorch_decoder':
            self.decoder_duration = nn.TransformerDecoder(nn.TransformerDecoderLayer(d_model=64, 
                                                                                     nhead=1,
                                                                                     activation='gelu',  
                                                                                     dropout=0.1,
        ...
        self.pos_embed_dur2 = SinusoidalPositionalEmbedding(embedding_dim=64, padding_idx=0, init_size=2 * 40)
...
class uvast_model(nn.Module):
    def __init__(self, args):
        ...
        self.dec_action = TransformerDecoder_UVAST(TransformerDecoderLayer_UVAST(d_model=64, 
                                                                                 nhead=1, 
                                                                                 activation='gelu', 
                                                                                 dropout=0.1, 
                                                                                 dim_feedforward=2048, args=args), 
                                                   repeat_mod=1, num_layers=2, args=args)

        # set up positional encoding and segment embedding
        if args.use_pe_tgt or args.use_pe_src:
            self.pos_embed = SinusoidalPositionalEmbedding(embedding_dim=64, padding_idx=0, init_size=2 * 40)
        self.dec_embedding = SegmentEmbedding(args)       
            
        # initialize alignment decoder for duration prediction
        self.prediction_action = nn.Linear(64, 40 + 2, bias=False)
    ...
    def forward(self, inputs, mask, seg_data=None, no_split_data=None):
        frames_to_segment_assignment = None
        pred_framewise, feat_enc = self.enc_feat(inputs, mask)
        for i in range(len(pred_framewise)):
            pred_framewise[i] = einops.rearrange(pred_framewise[i], 'B C E -> C B E')

        if seg_data is not None:
            tgt_emb_clsids, tgt_mask_from_pad = self.dec_embedding(seg_data)
            tgt_mask_from_pad = einops.rearrange(tgt_mask_from_pad, 'B C E -> B (C E)')
            tgt_emb_clsids = einops.rearrange(tgt_emb_clsids, 'B C E S -> B (C E) S')
            ...
            for iii in range(len(decoder_output)):
                ...
                pred_transcript = einops.rearrange(pred_transcript, 'B E (C T) -> C B E T', C = 40, T = 100)
            ...
            for i in range(len(pred_crossattn)):
                pred_crossattn[i] = einops.rearrange(pred_crossattn[i], 'B (C T) E -> C B T E', C = 40, T = 100)
            output = [pred_framewise, pred_transcripts, pred_crossattn, frames_to_segment_assignment]

            return output
            
        if seg_data is None:
            if self.args.extra_loss!=False:
                ...
                while seq[0, -1].item() != 1 and len(seq[0, :]) < 50:
                ...
                output = [pred_framewise, seq, dur[:, 1:], pred_dur_AD, pred_transcript_AD]
            else:
                output = pred_framewise
            return output
