...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_segments, log=True):
        ...
        if log:
            ...
            # print(f"target_classes_o max: {target_classes_o.max().item() if target_classes_o.numel() > 0 else 'empty'}")
            # print(f"target_classes_o min: {target_classes_o.min().item() if target_classes_o.numel() > 0 else 'empty'}")
            # print(f"outputs['pred_logits'] shape: {outputs['pred_logits'].shape}")

            # assert target_classes_o.max() < src_logits.shape[-1], \
            #     f"Class index {target_classes_o.max().item()} exceeds number of classes {src_logits.shape[-1]}"
            # assert src_logits.shape[0] == target_classes_o.shape[0], \
            #     f"Mismatch in batch size: {src_logits.shape[0]} vs {target_classes_o.shape[0]}"
