...
class PointtadDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, num_classes, num_querypoints, window_size, dim_feedforward=2048, nhead=8, dropout=0.1, activation="relu"):
        ...
        self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=0) #originally = drop out
        self.inst_parallel_mix = ParallelMixing(d_model, window_size, num_querypoints)

        self.linear1 = nn.Linear(d_model, dim_feedforward)
        self.dropout = nn.Dropout(0) #originally = drop out
        self.linear2 = nn.Linear(dim_feedforward, d_model)

        self.norm1 = nn.LayerNorm(d_model)
        self.norm2 = nn.LayerNorm(d_model)
        self.norm3 = nn.LayerNorm(d_model)
        self.dropout1 = nn.Dropout(0) #originally = drop out
        self.dropout2 = nn.Dropout(0) #originally = drop out
        self.dropout3 = nn.Dropout(0) #originally = drop out
    ...
    def forward(self, features, proposal_points, pro_features):
        ...
        pro_features = pro_features + pro_features2 #self.dropout1(pro_features2)
        ...
        pro_features = pro_features + cls_features #self.dropout2(cls_features)
        act_features = self.norm2(pro_features)

        act_features2 = self.linear2(self.dropout(self.activation(self.linear1(act_features))))
        act_features = act_features + act_features2 #self.dropout3(act_features2)
