    ...
    def predict(self, model_dir, results_dir, features_path, vid_list_file, epoch, actions_dict,
        ...
        with torch.no_grad():
            ...
            for vid in list_of_vids:
                ...
                light = np.loadtxt(args.dataroot + "/lights/" +  vid.split('.')[0] + '.csv', delimiter=',')
                if light.ndim == 1:
                    light = light[np.newaxis, :]  # (1, T)
                elif light.shape[1] != features.shape[1]:
                    light = light.T  # maybe it's (T, 1) → transpose to (1, T)

                num_frames = min(features.shape[1], light.shape[1])
                features = features[:, :num_frames]
                light = light[:, :num_frames]
                combined_features = np.concatenate((features, light), axis=0)  # (C+1, T)
