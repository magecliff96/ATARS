...
class Prediction_Generation(nn.Module):
    def __init__(self, num_layers, num_f_maps, dim, num_classes):
        ...
        self.conv_1x1_in = nn.Conv1d(dim+512, num_f_maps, 1)#edited +512 bc cos features
    ...
    def predict(self, model_dir, results_dir, features_path, vid_list_file, epoch, actions_dict,
        ...
        with torch.no_grad():
            ...
            for vid in list_of_vids:
                ...
                optic = np.load(args.dataroot + "/optic_clip_o/" +  vid.split('.')[0] + '.npy')
                # if optic.shape[1] != features.shape[1]:
                #     optic = optic.T  # maybe it's (T, 512) → transpose to (512, T)
                # 加载真实标签
                ...
                combined_features = np.concatenate((features, optic), axis=0)  # (C+512, T)
