...
class MViT(nn.Module):
    def __init__(self, num_actor_class,scale,mode='linear'):
        super(MViT, self).__init__()
        self.scale = scale
        self.model = mvit_base_16x4(True)
        # model 16 blocks
        cls = Head(768, num_actor_class)
    ...
    def forward(self, x): #[b, c, t, h, w]
        ...
        if isinstance(x, list):
            x = torch.stack(x, dim=1) #[b, v, 2048, h, w]
            # l, b, c, h, w
        ...
        return x #, attn_l # b,D_index, N-1,N-1

class VideoMAE_pretrained(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self,video_mae, num_actor_class):
        super(VideoMAE_pretrained, self).__init__()
        
        self.model = video_mae
        self.head  = Head(384, num_actor_class) #small 384 large 2x
    ...
    def forward(self, x):
        ...
        height, width = x[0].shape[2], x[0].shape[3]
        # if isinstance(x, list):
        #     x = torch.stack(x, dim=0) #[T, b, C, h, w] #[b, c, t, h, w]
        #     # l, b, c, h, w
        #     x = torch.permute(x, (1,2,0,3,4)) #[b, C, T, h, w]
        #x = rearrange(x, 'B C T H W -> (B T) C H W')
        x = self.model.forward_features(x)
        #x = rearrange(x, '(B T) C H W -> B C T H W')
        x  = self.head(self.model.fc_dropout(x))
        #x  = self.head(x)
