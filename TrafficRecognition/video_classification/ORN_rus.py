...
class ORN(Object_based):
    ...
    def forward(self, x, box=False):
        ...
        try:
            ...
            raise ValueError(f'Assertion failed: max_N: ({len(box[0])}) causing len(box) ({len(box)}) != B*T ({B}*{T} = {B*T})')

        features = self.extract_features(x) # b,d,t,H,W

        obj_features = self.get_object_features(features,box) # b,t,NFB & b,t,N,NFB
        all_e, all_is_obj = self.orn(obj_features,self.NFB)
        all_is_obj = all_is_obj.unsqueeze(-1)
        # all_e *= all_is_obj # b,t-1,(N)^2,NFB
