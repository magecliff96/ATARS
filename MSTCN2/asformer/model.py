...
from asformer.eval import segment_bars_with_confidence
...
class AttLayer(nn.Module):
    ...
    def construct_window_mask(self):
        ...
        # Use broadcasting to generate the indices for setting the mask
        indices = torch.arange(self.bl).view(-1, 1) + torch.arange(self.bl)
        window_mask[:, torch.arange(self.bl), indices] = 1

        # Move to the device after constructing the mask
