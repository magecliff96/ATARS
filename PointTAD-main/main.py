...
def main(args):
    # writer = SummaryWriter(log_dir=args.output_dir)  # Specify log directory
    utils.init_distributed_mode(args)
    print('git:\n  {}\n'.format(utils.get_sha()))
    print(args)

    if args.dataset not in ['multithumos', 'charades', 'carom']:
    ...
    if args.eval:
        # evaluator_t, eval_loss_dict_t = evaluate(model, criterion, postprocessors,
        #                                      data_loader_train, device, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        evaluator1, eval_loss_dict1 = evaluate(model, criterion, postprocessors,
                                             data_loader_val, device, args)
        res1 = evaluator1.summarize()
        # evaluator2, eval_loss_dict2 = evaluate(model, criterion, postprocessors,
        #                                      data_loader_train, device, args)
        # res2 = evaluator2.summarize()

        if utils.get_rank() == 0:
            results_pd = multi_formatting(res1, args.dataset)
...
def get_configs(dataset):
    yaml = YAML(typ='rt')  # Use 'rt' for round-trip parsing (preserves formatting)
    with open('datasets/dataset_cfg.yaml', 'r') as file:
        default_config = yaml.load(file)  # Load the YAML file
